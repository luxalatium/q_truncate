        ...
        for (unsigned int i1 = 1; i1 < BoxShape[0] - 1; i1 ++)  {
                ...
                for (unsigned int i2 = 1; i2 < BoxShape[1] - 1; i2 ++)  {
                        ...
                        for (unsigned int i3 = 1; i3 < BoxShape[2] - 1; i3 ++)  {
                                ...
                                PFdX1[i1][i2][i3] = std::abs(F[i1+1][i2][i3] - F[i1-1][i2][i3]) * coeff;
                        }
                }
        }        
        ...
        for (unsigned int i1 = 1; i1 < BoxShape[0] - 1; i1 ++)  {
                ...
                for (unsigned int i2 = 1; i2 < BoxShape[1] - 1; i2 ++)  {
                        ...
                        for (unsigned int i3 = 1; i3 < BoxShape[2] - 1; i3 ++)  {
                                ...
                                PFdX2[i1][i2][i3] = std::abs(F[i1][i2+1][i3] - F[i1][i2-1][i3]) * coeff;
                        }
                }
        }  
        ...
        for (unsigned int i1 = 1; i1 < BoxShape[0] - 1; i1 ++)  {
                ...
                for (unsigned int i2 = 1; i2 < BoxShape[1] - 1; i2 ++)  {
                        ...
                        for (unsigned int i3 = 1; i3 < BoxShape[2] - 1; i3 ++)  {
                                ...
                                PFdX3[i1][i2][i3] = std::abs(F[i1][i2][i3+1] - F[i1][i2][i3-1]) * coeff;
                        }
                }
        }  
		...
		t_1_end = omp_get_wtime();
			...
			t_1_end = omp_get_wtime();
				...
				g1 = TA[i] / M1;
                        ...
                        t_1_end = omp_get_wtime();
                        t_1_elapsed = t_1_end - t_1_begin;
                        //log->log("Elapsed time (omp-d-1 CASE 2 FF) = %lf sec\n", t_1_elapsed);
                        t_1_begin = omp_get_wtime();
                }   
			...
			t_1_end = omp_get_wtime();
                        t_1_elapsed = t_1_end - t_1_begin;
                        //log->log("Elapsed time (omp-d-1 CASE 3 FF full) = %lf sec\n", t_1_elapsed);
                }
