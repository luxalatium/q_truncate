...
void Scatter2d::Evolve()
{
    ...
    MeshD2D PFdX3(boost::extents[BoxShape[0]][BoxShape[1]]);
    PFdX3.reindex(0); 
    ...
    log->log("[Scatter2d] Elapsed time (initializing containers) = %lf sec\n\n", t_1_elapsed); 
    ...
    for (unsigned int i1 = 1; i1 < BoxShape[0] - 1 ; i1 ++)  {
        ...
        for (unsigned int i2 = 1; i2 < BoxShape[1] - 1 ; i2 ++)  {
            ...
            F[i1][i2] = Wavefunction_HH(Box[0] + i1 * H[0], Box[2] + i2 * H[1]);
        }
    }
    ...
    t_1_end = omp_get_wtime();
    t_1_elapsed = t_1_end - t_1_begin;
    log->log("[Scatter2d] Elapsed time (initializing wavefunction) = %lf sec\n\n", t_1_elapsed); 
    ...
    t_1_end = omp_get_wtime();
    t_1_elapsed = t_1_end - t_1_begin;
    log->log("[Scatter2d] Elapsed time (finite differences) = %lf sec\n\n", t_1_elapsed); 
    ...
    for (int tt = 0; tt < (int)(TIME / kk); tt ++)
    {
        ...
        while ( TBL.size() != 0 && !isFullGrid )
        {
            ...
            for (int i = 0; i < TA.size(); i++)  {
                ...
                KK1[g1][g2] = I * kh2m * (  Hisq[0] * ( F[g1 - 1][g2] - 2.0 * F[g1][g2] + F[g1 + 1][g2] )
                               ...
                               )  - I * k2hb * Potential_HH(xx1, xx2) * F[g1][g2];
            }
            ...
            for (int i = 0; i < TA.size(); i++)  {
                ...
                KK2[g1][g2] = I * kh2m * (  Hisq[0] * ( ( F[g1 - 1][g2] + 0.5 * KK1[g1 - 1][g2] ) - 2.0 * ( F[g1][g2] + 0.5 * KK1[g1][g2] ) + ( F[g1 + 1][g2] + 0.5 * KK1[g1 + 1][g2] ) )
                                ...
                                ) - I * k2hb * Potential_HH(xx1, xx2) * ( F[g1][g2] + 0.5 * KK1[g1][g2] );
            }
            ...
            for (int i = 0; i < TA.size(); i++)  {
                ...
                KK3[g1][g2] = I * kh2m * (  Hisq[0] * ( ( F[g1 - 1][g2] + 0.5 * KK2[g1 - 1][g2] ) - 2.0 * ( F[g1][g2] + 0.5 * KK2[g1][g2] ) + ( F[g1 + 1][g2] + 0.5 * KK2[g1 + 1][g2] ) )
                                ...
                                ) - I * k2hb * Potential_HH(xx1, xx2) * ( F[g1][g2] + 0.5 * KK2[g1][g2] );
            }
            ...
            for (int i = 0; i < TA.size(); i++)  {
                ...
                KK4[g1][g2] = I * kh2m * (  Hisq[0] * ( ( F[g1 - 1][g2] + 1.0 * KK3[g1 - 1][g2] ) - 2.0 * ( F[g1][g2] + 1.0 * KK3[g1][g2] ) + ( F[g1 + 1][g2] + 1.0 * KK3[g1 + 1][g2] ) )
                                ...
                                ) - I * k2hb * Potential_HH(xx1, xx2) * ( F[g1][g2] + 1.0 * KK3[g1][g2] );
            }
        ...
        if ( (tt + 1) % PERIOD == 0 )
        {
            ...
            log->log("[Scatter2d] Step: %d, Time = %f ACF = %.16e \n", tt + 1, kk * (tt + 1), norm);
            ...
            log->log("[Scatter2d] Step: %d, Elapsed time: %lf sec\n", tt + 1, t_0_elapsed);
...
inline std::complex<double> Scatter2d::Wavefunction_MO(double x1, double x2)
{
...
/* ------------------------------------------------------------------------------- */
inline double Scatter2d::Potential_MO(double x1, double x2)
{
